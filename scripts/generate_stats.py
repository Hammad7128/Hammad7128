...
def card(title, subtitle, body):
    ...
    <rect width="500" height="280" rx="16" fill="#022c22"/>
    <rect x="1" y="1" width="498" height="278" rx="15" fill="none" stroke="#065f46"/>
    <g font-family="Arial, sans-serif"><text x="26" y="40" fill="#6ee7b7" font-size="22" font-weight="700">{escape(title)}</text>
    <text x="26" y="64" fill="#a7f3d0" font-size="12">{escape(subtitle)}</text>
    {body}<text x="26" y="259" fill="#a7f3d0" font-size="11">Public GitHub data · Updated {date} UTC</text></g></svg>'''
...
def main():
    ...
    for i, (label, value) in enumerate(values):
        y = 104 + i * 36
        body += f'<circle cx="29" cy="{y-5}" r="3" fill="#34d399"/><text x="43" y="{y}" fill="#d1fae5" font-size="15">{label}</text><text x="467" y="{y}" text-anchor="end" fill="#6ee7b7" font-size="20" font-weight="700">{value}</text>'
    ...
    for i, (language, size) in enumerate(top):
        ...
        body += f'<text x="26" y="{y}" fill="#d1fae5" font-size="13">{escape(language)}</text><text x="474" y="{y}" text-anchor="end" fill="#6ee7b7" font-size="13">{percent:.1f}%</text><rect x="26" y="{y+6}" width="448" height="5" rx="2" fill="#064e3b"/><rect x="26" y="{y+6}" width="{448*percent/100:.2f}" height="5" rx="2" fill="#34d399"/>'
    if not top:
        body = '<text x="26" y="120" fill="#d1fae5" font-size="15">No language data yet.</text>'
